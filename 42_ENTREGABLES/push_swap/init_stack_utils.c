...
/*   Updated: 2024/10/31 17:14:11 by juagomez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* int	main(void)
{
	...
	stack_a_copy = stack_a; // COPIA PARA IMPRIMIR SIN MODIFICAR 
	LA DIRECCION DEL PTR *STACK
	while (stack_a_copy)
	{
		printf( "arg numero-> %d, \t prev-> %p \t address-> %p 
	...
	printf("TESTEO find_cheapest_node()-> ptr, value-> '%i' 
	cheapest_cost-> '%i' ptr-> '%p' \n\n", find_cheapest_node(stack_a)->value, 
	(stack_a)->next->cheapest_cost,  find_cheapest_node(stack_a));
